...
import { InstagramIcon, TwitterIcon, FaceBookIcon, IMDBIcon, WikiPediaIcon } from '../assets';
...
function Social({ externalIds, type, name, title }: SocialProps) {
    ...
    if (name) Wikipedia = `https://en.wikipedia.org/wiki/${name.replace(' ', '_')}`;

    if (title) Wikipedia = `https://en.wikipedia.org/wiki/${title.replace(' ', '_')}`;
        ...
        <Socials>
            ...
            <Wiki>
                <a href={Wikipedia} target="_blank" rel="noreferrer">
                    <img alt="Wikipedia" src={WikiPediaIcon} />
                </a>
            </Wiki>
        </Socials>
    );
