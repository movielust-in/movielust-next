...
function SimilarMovies({ id, type, title, toBeFiltered, genres, dom }: SimilarMoviesProps) {
        ...
        fetchSimilar(id, type, genres).then((response) => {
            if (type === 'movie' && toBeFiltered?.length) {
                setSimilar(
                    response!
                        .filter(
                            (movie) => !toBeFiltered.find((filtered) => filtered.id === movie!.id)!
                        )!
                        .slice(0, 21)!
                );
            } else {
...
export default memo(SimilarMovies, (prev, curr) => prev.id === curr.id && prev.type === curr.type);
