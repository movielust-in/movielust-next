...
const initialState: IntialStateType = {
    ...
    page: 1,
    totalPages: 0,
    filters: { genres: [], year: new Date().getFullYear(), sortBy: 'popularity.desc' },
...
const movieSlice = createSlice({
    ...
    reducers: {
        ...
        addMovies: (state, action) => {
            state.results = { ...state.results, [action.payload.page]: action.payload.results };
        },

        setSortByOption: (state, action) => {
            if (state.filters.sortBy !== action.payload) state.filters.sortBy = action.payload;
        },
