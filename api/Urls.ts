...
const POPULRAR_SERIES = (page: number) => `tv/popular?&language=en-US&page=${page}`;
...
const DETAIL = (id: string | number, type: string) =>
    `${type}/${id}?append_to_response=videos,credits,release_dates`;

const SHALLOW_DETAIL = (id: string, type: string) => `${type}/${id}`;

const EXTERNAL_IDS = (id: string | number) => `movie/${id}/external_ids`;

const TVEXTERNAL_IDS = (id: string | number) => `tv/${id}/external_ids?&language=en-US`;

const EPISODE_DETAIL = (id: string | number, season: string | number, episode: string | number) =>
...
const movieWithGenre = (genreId: string | number) => `discover/movie?&with_genres=${genreId}`;

const seriesWithGenre = (genreId: string | number) => `discover/tv?&with_genres=${genreId}`;
...
const PERSONMOVIES = (id: string | number) => `person/${id}/movie_credits?&language=en-US`;
...
const image = (width: string | number, src: string) => `https://image.tmdb.org/t/p/w${width}${src}`;

const PEOPLE_EXTERNALIDS = (id: string | number) => `person/${id}/external_ids?&language=en-US`;
...
const SEARCH = (query: string) => `search/multi?&query=${query}&include_adult=false`;

const TVIMAGES = (id: string | number, type: string) => `${type}/${id}/images`;

const WATCH_PROVIDERS = (id: string | number, type: string) => `${type}/${id}/watch/providers`;

const SEASON = (id: string | number, season: string | number) =>
    `tv/${id}/season/${season}?&language=en-US`;

const TAGGED_IMG = (id: string | number) => `person/${id}/tagged_images?&language=en-US&page=1`;
...
const CASTTV = (id: string | number) => `person/${id}/tv_credits?&language=en-US`;
...
const VERIFY_OTP = (email: string, otp: string | number, otp_type: string | number) =>
    `${SERVER_URI}/auth/verifyotp/${email}/${otp}/${otp_type}`;

const UPDATE_AVATAR = (avatarId: string | number) => `${SERVER_URI}/user/update/avatar/${avatarId}`;
...
const HINDI_TORRRENT = (id: string | number, title: string, releaseYear: string | number) => {
    title = title
        .replace('-', '')
        .replace('  ', ' ')
        .replace(/[&/\\#,+()$~%.'":*?<>{}]/g, '');
...
const IMDB_RATING = (imdb_id: string) => `${SERVER_URI}/movie/imdb-rating/${imdb_id}`;

const IMDB_RATINGS = (ids: string[]) => `${SERVER_URI}/movie/imdb-ratings/${ids.join(',')}`;
...
const MAGNET = (id: string | number, title: string, releaseYear: string | number) => {
    title = title.replace(/[&/\\#,+()$~%.'":*?<>{}]/g, '');
