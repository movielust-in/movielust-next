...
export const discoverMovie = (filters: Filters, page: number) => {
    let params: {} = { page, with_genres: filters.genres.join(','), sort_by: filters.sortBy };

    if (filters.sortBy === 'vote_average.desc')
        params = { ...params, 'vote_count.gte': filters.genres.length ? 1500 : 1500 };
    else if (filters.sortBy === 'release_date.desc')
        params = { ...params, 'release_date.lte': new Date(), 'vote_count.gte': 50 };
...
export const fetchTRM = () =>
    new Promise((resolve) => {
        getAll([TRM(1), TRM(2), TRM(3), TRM(4), TRM(5)], tmdbClient).then((response) => {
            const results = [
...
export const fetchExternalIds = (id: string | number): Promise<MovieExternalIdsResponse> =>
    new Promise((resolve, reject) => {
        (async () => {
            try {
                const res = await tmdbClient.get<MovieExternalIdsResponse>(EXTERNAL_IDS(id));
                resolve(res.data);
            } catch (err) {
                reject(err);
            }
...
export const CompaniesTopImages = (id: string | number): Promise<MovieResult[]> =>
    new Promise((resolve, reject) => {
        (async () => {
            try {
...
export const fetchCompanyMovies = (id: string | number): Promise<AllResponse<MovieResult[]>> =>
    fetchAllPages((page: number) => COMPANYMOVIES(id, page));

// All South Movies
export const fetchallSouth = () => fetchAllPages((page: number) => SOUTHALL(page));

// Person All Movies
export const fetchCastMovies = (id: string | number): Promise<AllResponse<MovieResult[]>> =>
...
// All Gujarati Movies
export const fetchallGujarati = () => fetchAllPages((page: number) => GUJARATIALL(page));
